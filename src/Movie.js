...
function _renderGenres(genre_array, key) {
  const result = genre_array.map((item, index) => {
    return (
      <li className="Movie__Genre" key={`${key}_genre_${index + 1}`}>
        {item}
      </li>
    );
  });
...
function Movie({ title, poster, genres, overview, parentKey }) {
  return (
    <div className="Movie">
...
function MoviePoster({ poster }) {
  return <img className="Movie__Poster" src={poster} width="100%" alt="" />;
}

function MovieGenres({ genres, parentKey }) {
  return (
    <div className="Movie__Genres">
      <ul aria-label="Movie Genres">{_renderGenres(genres, parentKey)}</ul>
    </div>
  );
}
